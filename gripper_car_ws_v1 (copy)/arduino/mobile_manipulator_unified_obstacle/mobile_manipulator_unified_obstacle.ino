  ...
  if (motor_state[motor_idx] != prev_motor_state[motor_idx]) {
    if ((prev_motor_state[motor_idx] == 1 && motor_state[motor_idx] == -1) ||
      ...
      if (!test_mode) {
        ...
        motor_state[motor_idx] = 0;
        Serial.print("STATUS,Motor "); Serial.print(motor_idx); Serial.println(" locked");
      }
    }
...
void scanI2C() {
  Serial.println("STATUS,Scanning I2C bus...");
  byte count = 0;
  for (byte addr = 1; addr < 127; addr++) {
    Wire.beginTransmission(addr);
    if (Wire.endTransmission() == 0) {
      Serial.print("STATUS,I2C device at 0x");
...
void setup() {
  ...
  // Scan I2C to verify PCA9685 is connected
  scanI2C();
  ...
  // Test: sweep servo 0 on startup
  Serial.println("STATUS,Testing servo 0...");
  goServoAngle(0, 45);
  delay(500);
  goServoAngle(0, 135);
  delay(500);
  goServoAngle(0, 90);
  delay(500);
  Serial.println("STATUS,Servo 0 test complete");
  ...
  for (int i = 0; i < 6; i++) goServoAngle(SERVO_CH[i], armCurrentAngle[i]);
