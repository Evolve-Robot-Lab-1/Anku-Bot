...
const CONFIG = {
    ...
    obstacleStatusTopic: '/obstacle_avoidance/status'
};

// Arm pose definitions (in radians for serial_bridge conversion)
// Joint order: [j1-Shoulder, j2-Elbow, j3-Wrist, j4-BaseRotation, gripper, left_gear]
const ARM_POSES = {
    // Home: Arm folded/compact
    home: [0, 0.524, 1.57, -1.57, -2.09, 0],
    
    // Search: Arm up and forward to look around
    search: [0, 0, 0, 0, -2.09, 0],
    
    // Pick: Arm extended down to grab objects (base at 0°, gripper open)
    pick: [0, -0.785, 0.785, 0, 0, 0],
    
    // Carry: User-defined position (gripper closed)
    carry: [1.57, -0.89, -0.052, -0.855, -2.09, 0.15],
    
    // Place: User-defined position (gripper open)
    place: [-1.34, -1.20, -0.087, -1.33, 0, 0.15]
};

// Gripper: CH4 (gripper_base_joint) - closed=30deg, open=90deg
const GRIPPER = {
    open: 0,        // 90 degrees servo angle
    closed: -2.09   // 30 degrees servo angle
};
...
let currentJointPositions = [0, 0.524, 1.57, -1.57, -2.09, 0];  // Track current arm positions
...
function setupSubscribers() {
    ...
    armStateSub.subscribe((msg) => {
        ...
        // Update sliders with current arm positions
        if (msg.position && msg.position.length > 0) {
            updateSlidersFromState(msg.position);
        }
    });
    ...
    scanSub.subscribe((msg) => {
        ...
        // Only update if not getting status from obstacle avoidance node
        if (!elements.statusObstacle.dataset.fromNode) {
    ...
    // Subscribe to obstacle avoidance node status (more detailed)
    const obstacleStatusSub = new ROSLIB.Topic({
        ros: ros,
        name: CONFIG.obstacleStatusTopic,
        messageType: 'std_msgs/String'
    });
    obstacleStatusSub.subscribe((msg) => {
        // Parse status: "state:driving,front:0.81,left:2.25,right:0.79,attempts:0"
        const data = {};
        msg.data.split(',').forEach(item => {
            const [key, val] = item.split(':');
            data[key] = val;
        });

        elements.statusObstacle.dataset.fromNode = 'true';
        const state = data.state || 'unknown';
        const front = parseFloat(data.front) || 0;

        // Update display based on state
        const stateColors = {
            'driving': '#4CAF50',           // Green
            'obstacle_detected': '#f44336', // Red
            'reversing': '#ff9800',         // Orange
            'stopped': '#ff9800',           // Orange
            'scanning': '#2196F3',          // Blue
            'turning': '#2196F3',           // Blue
            'check_clear': '#ff9800'        // Orange
        };

        const stateLabels = {
            'driving': `Clear (${front.toFixed(2)}m)`,
            'obstacle_detected': `OBSTACLE! (${front.toFixed(2)}m)`,
            'reversing': 'REVERSING',
            'stopped': 'STOPPED',
            'scanning': 'SCANNING',
            'turning': 'TURNING',
        ...
        elements.statusObstacle.textContent = stateLabels[state] || state;
        elements.statusObstacle.style.color = stateColors[state] || '#666';
    });
...
function setupEventListeners() {
    ...
    // Cruise Control
    document.getElementById('btn-cruise-forward').addEventListener('click', startCruiseForward);
    document.getElementById('btn-cruise-stop').addEventListener('click', stopCruise);
...
function startCruiseForward() {
    if (eStopActive) return;

    // Stop any existing cruise
    stopCruise();

    cruiseActive = true;
    const btn = document.getElementById('btn-cruise-forward');
    btn.classList.add('cruise-active');
    btn.textContent = '▲ CRUISING...';

    // Send velocity command every 100ms
    cruiseInterval = setInterval(() => {
        if (cruiseActive && !eStopActive) {
            sendVelocity(currentSpeed, 0);
            elements.statusLinearVel.textContent = currentSpeed.toFixed(2);
            elements.statusAngularVel.textContent = '0.00';
        }
    }, 100);
...
function stopCruise() {
    ...
    const btn = document.getElementById('btn-cruise-forward');
    btn.classList.remove('cruise-active');
    btn.textContent = '▲ Cruise Forward';
...
function toggleEStop() {
    ...
    if (eStopActive) {
        stopCruise();  // Stop cruise control too
        stopMoving();
...
function sendArmPose(poseName) {
    ...
    currentJointPositions = pose.slice();
...
function sendGripper(position) {
    ...
    // Update ONLY the gripper position (index 4) in tracked positions
    currentJointPositions[4] = position;

    // Send ALL joints to prevent arm from moving to default positions
    const msg = new ROSLIB.Message({
        header: {
            stamp: { sec: 0, nanosec: 0 },
            frame_id: ''
        ...
        name: JOINT_NAMES,
        position: currentJointPositions.slice(),
        velocity: [],
        effort: []
    });

    armCommandPublisher.publish(msg);
    elements.statusArm.textContent = position > -1 ? 'Gripper Open' : 'Gripper Closed';
}
...
// Joint slider functions
function setupJointSliders() {
    const sliders = ['j1', 'j2', 'j3', 'j4', 'gripper'];

    sliders.forEach(joint => {
        const slider = document.getElementById(`slider-${joint}`);
        const valueSpan = document.getElementById(`${joint}-value`);
    ...
    // Send joints button (still available for manual send)
    const sendBtn = document.getElementById('btn-send-joints');
    if (sendBtn) {
        sendBtn.addEventListener('click', sendJointPositions);
    }
}

function sendJointPositions() {
    if (!armCommandPublisher) return;

    // Get slider values (degrees) and convert to radians
    const j1 = parseFloat(document.getElementById('slider-j1').value) * Math.PI / 180;
    const j2 = parseFloat(document.getElementById('slider-j2').value) * Math.PI / 180;
    const j3 = parseFloat(document.getElementById('slider-j3').value) * Math.PI / 180;
    const j4 = parseFloat(document.getElementById('slider-j4').value) * Math.PI / 180;
    const gripperPercent = parseFloat(document.getElementById('slider-gripper').value);
    const gripper = gripperPercent / 100.0;  // Convert 0-100% to 0-1

    // Track current positions (keep gripper at index 4 as current)
    currentJointPositions = [j1, j2, j3, j4, currentJointPositions[4], gripper];
    ...
    armCommandPublisher.publish(msg);
    elements.statusArm.textContent = 'Manual';
    console.log('Sent joint positions:', currentJointPositions);
}

// Update sliders from arm state feedback
function updateSlidersFromState(positions) {
    if (positions.length >= 5) {
        document.getElementById('slider-j1').value = Math.round(positions[0] * 180 / Math.PI);
        document.getElementById('j1-value').textContent = Math.round(positions[0] * 180 / Math.PI);

        document.getElementById('slider-j2').value = Math.round(positions[1] * 180 / Math.PI);
        document.getElementById('j2-value').textContent = Math.round(positions[1] * 180 / Math.PI);

        document.getElementById('slider-j3').value = Math.round(positions[2] * 180 / Math.PI);
        document.getElementById('j3-value').textContent = Math.round(positions[2] * 180 / Math.PI);

        document.getElementById('slider-j4').value = Math.round(positions[3] * 180 / Math.PI);
        document.getElementById('j4-value').textContent = Math.round(positions[3] * 180 / Math.PI);

        if (positions.length >= 6) {
            const gripperPercent = Math.round(positions[5] * 100);
            document.getElementById('slider-gripper').value = gripperPercent;
            document.getElementById('gripper-value').textContent = gripperPercent;
        }
    }
}
