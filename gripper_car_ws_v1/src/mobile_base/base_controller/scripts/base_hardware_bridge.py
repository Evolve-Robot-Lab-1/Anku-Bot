...
Topics:
  ...
  Publish:
    ...
    - /base/joint_states (sensor_msgs/JointState): Wheel positions
...
class BaseHardwareBridge(Node):
    ...
    def __init__(self):
        ...
        self.obstacle_sub = self.create_subscription(
            Bool, '/obstacle_detected', self.obstacle_callback, qos_profile)

        # Publishers
        self.odom_pub = self.create_publisher(Odometry, '/odom', qos_profile)
        self.joint_state_pub = self.create_publisher(JointState, '/base/joint_states', qos_profile)
    ...
    def obstacle_callback(self, msg):
        """Handle /obstacle_detected flag"""
        obstacle_flag = 1 if msg.data else 0
        command = f"OBS,{obstacle_flag}"
        self.send_command(command)
        self.get_logger().info(f'Obstacle flag: {obstacle_flag}')
