...
#define SERVO_STEP_DELAY 50  // milliseconds for smooth servo motion (slower)
...
const uint8_t SERVO_CH[6] = {0, 1, 2, 3, 4, 5};  // Match physical wiring: Ch0-4 for 5 servos
...
// Arm joint state - Home position: folded/sitting
// CH0=90, CH1=120, CH2=180, CH3=0, CH4=30(gripper closed), CH5=90
float armCurrentAngle[6] = {90, 120, 180, 0, 30, 90};
float armTargetAngle[6] = {90, 120, 180, 0, 30, 90};
float armLastSentAngle[6] = {90, 120, 180, 0, 30, 90};
...
const int motor_direction[4] = { -1, -1, -1, -1 };  // Flipped: LiDAR side is now "forward"
const int encoder_dir[4] = { -1, 1, -1, 1 };  // Flipped to match motor direction
...
void processCommand(String cmd) {
  ...
  // Also accepts SET_ALL_SERVOS for compatibility with servo_bridge
...
void setup() {
  ...
  delay(100);  // Brief delay instead of blocking while(!Serial)

  // Initialize I2C and PCA9685
  Wire.begin();
  pca.begin();
  pca.setPWMFreq(SERVO_FREQ_HZ);
  delay(500);  // PCA9685 needs time to initialize
