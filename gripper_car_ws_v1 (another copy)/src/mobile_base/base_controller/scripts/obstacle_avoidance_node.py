...
class SmartTurnObstacleAvoidance(Node):
    ...
    def _analyze_scan_sectors(self, msg: LaserScan):
        ...
        self.left_sector_avg = (sum(left_readings) / len(left_readings)) if left_readings else float('inf')
        self.right_sector_avg = (sum(right_readings) / len(right_readings)) if right_readings else float('inf')
    ...
    def _state_driving(self):
        ...
        # Check for obstacle - trigger if:
        # 1. Moving forward (original behavior), OR
        # 2. Obstacle is VERY close (emergency stop regardless of command)
        if self.min_front_distance < self.obstacle_threshold:
            emergency_threshold = self.obstacle_threshold * 0.7  # 70% of threshold
            if self.latest_cmd.linear.x > 0 or self.min_front_distance < emergency_threshold:
                self.get_logger().warn(
                    f'OBSTACLE at {self.min_front_distance:.2f}m - initiating avoidance'
                )
                self.avoidance_attempts = 0
                # Publish obstacle detected flag
                obstacle_msg = Bool()
    ...
    def _state_turning(self):
        ...
        # BUT: Give robot at least 1 second to actually turn before checking
        # This prevents immediate abort in confined spaces
        if elapsed >= 1.0 and self.min_front_distance < self.obstacle_threshold * 0.5:
